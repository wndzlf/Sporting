...
//                    //데이터베이스를 불러온후
//                    _ = Database.database().reference(fromURL: "https://realsporting-d74ae.firebaseio.com/")
//                    //let rooomReference = refDB
//
//                    guard let roomTeamName = self.teamName.text else {return}
//
//                    let values = ["roomPicture":imageURL,"roomPlace":"서울시 중랑구 신내동(google Apl 사용 요망)","roomDate-time":timeStamp,"roomNotification":"초보자들 안받아요 못하면 직접 뛰든가", "roomSports":roomSports, "roomCaptainUID":roomCaptainUID ,"roomTeamName":roomTeamName] as [String : Any]                            //현재 유저의 UID의 참조값에 child에 저장한다.
